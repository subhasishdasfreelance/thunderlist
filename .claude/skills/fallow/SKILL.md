---
name: fallow
description: 'MANUAL INVOCATION ONLY - never load or run this skill on your own initiative, and never as part of another task. Use it only when the user explicitly names fallow, for example "use fallow to check code health", "run fallow", or "fallow audit". It is codebase intelligence for TypeScript and JavaScript - unused code and dependencies, duplication, circular dependencies, complexity hotspots, architecture boundary violations, changed-code risk, and supported auto-fixes. Do NOT infer it from general requests to refactor, clean up, review, or improve code.'
license: MIT
---
